...
fn value_to_limbs<E: Engine, F: PrimeField>(
    ...
    match value {
        Some(value) => {
            ...
            let binary_limbs: Vec<Option<E::Fr>> = binary_limb_values
                .into_iter()
                .map(|el| Some(biguint_to_fe::<E::Fr>(el)))
                .collect();
...
impl<'a, E: Engine, F: PrimeField> FieldElement<'a, E, F> {
    ...
    pub fn enforce_is_normalized<CS: ConstraintSystem<E>>(
        ...
        for r in results.into_iter() {
            ...
            let _ =
                create_range_constraint_chain(cs, &el, params.binary_limbs_params.limb_size_bits)?;
        }
    ...
    pub(crate) fn compute_congruency<CS: ConstraintSystem<E>>(
        ...
        let represented_modulus_modulo_base = Term::<E>::from_constant(biguint_to_fe(
            params.represented_field_modulus.clone() % &params.base_field_modulus,
        ));
    ...
    pub(crate) fn get_congruency_class(&self) -> u64 {
        ...
        println!(
            "From value = {}, from limb = {}",
            from_value.to_str_radix(16),
            from_limb.to_str_radix(16)
        );
...
mod test {
    ...
    fn test_compute_congruency() {
        ...
        let a_1 = FieldElement::new_allocated(&mut cs, Some(a_f_1), &params).unwrap();

        let a_2 = FieldElement::new_allocated(&mut cs, Some(a_f_2), &params).unwrap();

        let a_3 = FieldElement::new_allocated(&mut cs, Some(a_f_3), &params).unwrap();

        let b_f_1 = biguint_to_fe::<Fq>(BigUint::from(7u32));

        let b_1 = FieldElement::new_allocated(&mut cs, Some(b_f_1), &params).unwrap();

        let b_f_2 = biguint_to_fe::<Fq>(BigUint::from(5u32) + params.base_field_modulus.clone());

        let b_2 = FieldElement::new_allocated(&mut cs, Some(b_f_2), &params).unwrap();
        ...
        println!(
            "The term of compute_congruency of {} with {} is {}",
            a_f_1,
            b_f_1,
            test_result_congruence_class_11.unwrap()
        );
        println!(
            "The term of compute_congruency of {} with {} is {}",
            a_f_1,
            b_f_2,
            test_result_congruence_class_12.unwrap()
        );
        println!(
            "The term of compute_congruency of {} with {} is {}",
            a_f_2,
            b_f_1,
            test_result_congruence_class_21.unwrap()
        );
        println!(
            "The term of compute_congruency of {} with {} is {}",
            a_f_2,
            b_f_2,
            test_result_congruence_class_22.unwrap()
        );
        println!(
            "The term of compute_congruency of {} with {} is {}",
            a_f_3,
            b_f_1,
            test_result_congruence_class_31.unwrap()
        );
        println!(
            "The term of compute_congruency of {} with {} is {}",
            a_f_3,
            b_f_2,
            test_result_congruence_class_32.unwrap()
        );
    ...
    >(
        ...
        for i in 0..100 {
            ...
            let b_const = FieldElement::new_constant(b_f, &params);
